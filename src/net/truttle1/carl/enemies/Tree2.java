...
public class Tree2 extends GameObject{
	...
	public Tree2(Game window, int x, int y) {
		super(window);
		//Sets up the enemy's location, animation, id, and speed.
		this.x = x*100;
		this.y = y*100;
		this.currentAnimation = Sprites.tree2();
	...
	private void runWhileAlive()
	{
		if(!dead && hitTime<=0)
		{
			...
			{
				this.setFrame(0, 0);
				this.setAnimation(Sprites.treeShoot2());
				counter = 0;
			}
			if(this.currentAnimation.equals(Sprites.treeShoot2()) && this.getFrame(0) == 12)
			{
				if(flipped)
				{
			...
			if(this.currentAnimation.equals(Sprites.treeShoot2()) && this.getFrame(0)>=23)
			{
				this.currentAnimation = Sprites.tree2();
				counter = 0;
			}
	...
	private void playerCollision(Carl carl)
	{
		...
		if(carl.getAttackDamage()<2 && this.getBoundsDamage().intersects(carl.attackBounds()) && carl.getAttack()>0 && carl.getAttack()<9999)
		{
			this.carlHasTie = carl.getTie();
			this.setFrame(0, 0);
			this.setAnimation(Sprites.treeHit2());
			this.hitTime = 24;
		}
		else if(carl.getAttackDamage()>=2 && this.getBoundsDamage().intersects(carl.attackBounds()) && carl.getAttack()>0 && carl.getAttack()<9999)
		{
			this.carlHasTie = carl.getTie();
			die();
		}
