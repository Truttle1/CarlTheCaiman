...
public class Sand extends Grass{
	...
	public Sand(int x, int y,Game window,OverworldMode om) {
		super(x,y,window,om);
		unmodifiedX = x;
		unmodifiedY = y;
		this.x = x*100;
		...
		if(topLine && bottomLine && leftLine && rightLine)
		{
			for(int i=0; i<om.getObjects().size();i++)
			{
				if(om.getObjects().get(i) != null && om.getObjects().get(i).getId() == ObjectId.Ground)
				{
					...
					if(g != null && g != this && (g instanceof Sand || g instanceof Stone))
					{
