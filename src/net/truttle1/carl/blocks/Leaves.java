...
public class Leaves extends Grass{
	...
	Color clr = Color.green.darker();
	public Leaves(int x, int y,Game window,OverworldMode om) {
		super(x,y,window,om);
		unmodifiedX = x;
		unmodifiedY = y;
		this.x = x*100;
	...
	public void tick() {
		if(topLine && bottomLine && leftLine && rightLine)
		{
			...
			{
				if(om.getObjects().get(i) != null && om.getObjects().get(i).getId() == ObjectId.Cloud)
				{
					Grass g = (Grass)om.getObjects().get(i);
					if(g != null && g != this && g instanceof Leaves)
					{
